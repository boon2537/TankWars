...
public:
	...
	Vector2			EvaluateAtCumulativeParametric( float t ) const; //hint: get correct segment then to EvaluateCubicBezier
...
inline T EvaluateQuadraticBezier(const T & startPos, const T & actualCurveMidPos, const T & endPos, float t)
{
	T a = startPos;
	T c = endPos;
	T b = (2 * actualCurveMidPos) - (0.5f * startPos) - (0.5f * endPos);
