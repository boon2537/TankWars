...
std::map<std::string, AudioGroup*> AudioSystem::s_audioGroups;
...
void AudioSystem::LoadAudioGroups(const std::string& path)
{
	...
	while (elementToUse != nullptr)
	{
		...
		if (elementName == "Clip")
		{
			...
			SoundID id = g_audioSystem->CreateOrGetSound(ParseXmlAttribute(*elementToUse, "path", "empty"));
			g_audioSystem->m_audioClips[name] = id;
		}
		else if (elementName == "Group")
		{
			std::string name = ParseXmlAttribute(*elementToUse, "name", "empty");
			s_audioGroups[name] = new AudioGroup(*elementToUse);
		}
...
SoundPlaybackID AudioSystem::PlayOneOff(const std::string& name, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	return s_audioGroups[name]->PlayOneOff(isLooped, volume, balance, speed, isPaused);
}

SoundPlaybackID AudioSystem::PlayClipOneOff(const std::string& clipName, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
// 	std::map<std::string, SoundID>::iterator found = g_audioSystem->m_audioClips.find( clipName );
// 	if( found != g_audioSystem->m_audioClips.end() )
// 	{
// 		return found->second;
// 	}

	return g_audioSystem->PlaySound(g_audioSystem->m_audioClips[clipName], isLooped, volume, balance, speed, isPaused);
}

SoundID AudioSystem::FindClip(const std::string& clipName)
{
	std::map<std::string, SoundID>::iterator found = g_audioSystem->m_audioClips.find( clipName );
	if( found != g_audioSystem->m_audioClips.end() )
	{
